...
class ToDoListTableViewController: UITableViewController, MyProtocol {
    ...
    override func viewDidLoad() {
        ...
        self.tasks = [] // GET TASKS STORED SOMEWHERE or just empty
    ...
    */
    ...
    override func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        if !self.tasks[indexPath.row].isComplete() {
            ...
            let action = UIAlertAction(title: "Mark as complete", style: .Default) { _ in
                let text = tableView.cellForRowAtIndexPath(indexPath)?.textLabel?.text
                let cell = tableView.cellForRowAtIndexPath(indexPath)
                cell?.textLabel?.text = text! + " - Complete"
                self.tasks[indexPath.row].complete = true
            }
            let alert = UIAlertController(title: "Task", message:tableView.cellForRowAtIndexPath(indexPath)?.textLabel?.text, preferredStyle: .Alert)
